...
PairIPLCutSmooth::PairIPLCutSmooth(LAMMPS *lmp) :
...
PairIPLCutSmooth::~PairIPLCutSmooth()
{
  if (allocated) {
...
void PairIPLCutSmooth::compute(int eflag, int vflag)
{
...
void PairIPLCutSmooth::allocate()
{
  allocated = 1;
  int n = atom->ntypes + 1;
...
void PairIPLCutSmooth::settings(int narg, char **arg)
{
  if (narg != 2) error->all(FLERR, "Illegal pair_style command");

  powern     = utils::numeric(FLERR, arg[0], false, lmp);
  cut_global = utils::numeric(FLERR, arg[1], false, lmp);

  // set c0, c2, c4
  double rcinv = 1.0 / cut_global;
  double dpn   = (double)powern;
  c0 = -1.0 * (dpn+2.0)*(dpn+4.0)/8.0*powint(rcinv, powern);
  c2 =         dpn     *(dpn+4.0)/4.0*powint(rcinv, powern+2);
  c4 = -1.0 *  dpn     *(dpn+2.0)/8.0*powint(rcinv, powern+4);
...
void PairIPLCutSmooth::init_style()
{
  Pair::init_style();
...
double PairIPLCutSmooth::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairIPLCutSmooth::write_restart(FILE *fp)
{
...
void PairIPLCutSmooth::read_restart(FILE *fp)
{
...
void PairIPLCutSmooth::write_restart_settings(FILE *fp)
{
  fwrite(&cut_global, sizeof(double), 1, fp);
  fwrite(&powern,     sizeof(int), 1, fp);
...
void PairIPLCutSmooth::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
    utils::sfread(FLERR, &cut_global, sizeof(double), 1, fp, nullptr, error);
    utils::sfread(FLERR, &powern,     sizeof(int), 1, fp, nullptr, error);
    utils::sfread(FLERR, &c0,         sizeof(double), 1, fp, nullptr, error);
    utils::sfread(FLERR, &c2,         sizeof(double), 1, fp, nullptr, error);
    utils::sfread(FLERR, &c4,         sizeof(double), 1, fp, nullptr, error);
    utils::sfread(FLERR, &offset_flag, sizeof(int), 1, fp, nullptr, error);
    utils::sfread(FLERR, &mix_flag, sizeof(int), 1, fp, nullptr, error);
  }
  MPI_Bcast(&cut_global, 1, MPI_DOUBLE, 0, world);
  MPI_Bcast(&powern, 1, MPI_INT, 0, world);
...
void PairIPLCutSmooth::write_data(FILE *fp)
{
...
void PairIPLCutSmooth::write_data_all(FILE *fp)
{
...
double PairIPLCutSmooth::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairIPLCutSmooth::extract(const char *str, int &dim)
{
  dim = 2;
